...
The application is local-first and analysis-only. React/Vite/TypeScript renders a mock-default dashboard while the Node service owns read-only OANDA access and SQLite immutable reports. OANDA credentials remain server-side. Manual OANDA analysis uses separate completed H4 and Daily inputs: Daily is for Daily Regime, H4 is for structure and decisions. Saved reports include non-sensitive immutable display snapshots and can be opened from Analysis History. Chart Preview is on-demand and read-only, and can export itself as a PNG. The scheduler is in-process, defaults to fixture mode, OANDA mode is explicit opt-in, evaluates all six daily H4-close slots, and its OANDA fetch retries with backoff without ever accepting a stale/wrong-window H4 candle. Cross-market confirmation (US500/US30/Russell 2000) is live, fetched from the same OANDA account. Event-risk is a validation spike (unofficial feed), not a resolved production input; since ADR-018, entry authorization from the OANDA pipeline is no longer hard-blocked on it — the pipeline persists and displays its real computed decision, and the trader weighs the still-unofficial event-risk data themselves (see `docs/DECISIONS.md` ADR-016/ADR-018). Scheduler outcomes trigger a local, informational-only OS notification. The browser never connects to OANDA. Strategy parameters are configurable and versioned, and an isolated CLI backtest harness (`scripts/backtest/`) replays the real decision pipeline against OANDA history with a zero-lookahead guarantee (ADR-017); no live/scheduled run selects a non-default strategy yet.
...
Start with manual mock refresh. Later, use a single scheduler inside the local service. The official application timezone is `America/Toronto`, the primary timeframe is H4, and scheduled analyses run six times daily — once per H4 close, at 01:01, 05:01, 09:01, 13:01, 17:01, and 21:01 in that timezone (see ADR-012). These times are one minute after each expected H4 close to reduce incomplete-candle risk. It should:
...
The scheduler must use an idempotency key such as `instrument:timeframe:completedCandleAt:strategyVersion`. A scheduled clock tick is not evidence that a candle closed. The provider-confirmed closed status and completed-candle timestamp are authoritative. An open, unavailable, stale, or inconsistent latest candle cannot produce `BUY` or `SELL`. Since ADR-017, `oandaRunKey` (`src/service/oandaRun.ts`) also appends a `strategyConfigId` segment (`default` when no strategy is selected, which is every live/scheduled run today) so two strategies analyzing the same candle close never collide on the same key.

## SQLite strategy

SQLite is local-only and accessed only by the service. It uses migrations (an additive `CREATE TABLE IF NOT EXISTS` / `PRAGMA table_info` column-check pattern gated by a bumped `PERSISTENCE_SCHEMA_VERSION`, not a full migration runner), WAL mode, busy timeout, and transactional persistence (`src/persistence/analysisRepository.ts`). Implemented tables: `analysis_reports` (immutable report JSON plus queryable summary columns — action, score, instrument, timeframe, completed-candle timestamp), `analysis_runs` (run status, source, `run_key` uniqueness for idempotency, and since ADR-017 an optional `strategy_config_id` reference), `strategy_configs` (versioned strategy parameter sets, ADR-017), and `schema_migrations`. `application_settings`, `provider_status`, `notification_log`, and `export_log` remain proposed, not yet implemented.
...
`scripts/backtest/` is an isolated CLI tool, not part of the production service process, for measuring win rate/R:R/expectancy of a strategy against OANDA history (ADR-017). It reuses the exact production decision functions (`buildOandaMultiTimeframeInputs`, `buildDashboardState`) rather than reimplementing them, and persists to its own SQLite file (`backtest-results.sqlite`, `scripts/backtest/backtestRepository.ts`) separate from `analysis_reports.sqlite`, read read-only by the running web service to serve `GET /backtests*`. Its own `tsconfig.scripts.json` (referenced from the root `tsconfig.json`) keeps it type-checked without folding it into the app's `src/` build. Historical candles are cached locally as flat JSON (`scripts/backtest/.cache/`, gitignored) via a chunked `from`/`to` backfill against OANDA's per-request candle cap. See ADR-017 for the zero-lookahead guarantee, the fill/outcome state machine, and the CLI-only trigger decision.
