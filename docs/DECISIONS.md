...
Status: **Superseded by ADR-018.** Originally accepted as temporary — revisit once ADR-014's event-risk provider question is resolved. Kept below for history; the clamp itself no longer exists in the code.

`enforceAnalysisSafety` (`src/domain/safety.ts`, ADR-015) only runs on the browser/chart-adapter side (`src/components/chart/chartAdapter.ts`); the OANDA server pipeline (`src/service/oandaRun.ts`) never calls it. The sole entry-authorization gate for that pipeline was `safetyConstrainedState`, which unconditionally forced `action` to `WAIT`-family, `isActionable: false`, and cleared entry/stop/target fields regardless of what the underlying deterministic pipeline (`buildDashboardState`) computed. This was originally redundant — with cross-market and event-risk both hardcoded `UNAVAILABLE`/`unknown`, the Patience Filter alone already prevented `allowed` status — but ceased to be redundant once A1 made cross-market confirmation real. The clamp was kept in place regardless: as long as event-risk remained a validation spike (ADR-014) rather than a resolved production input, no realistic combination of inputs could authorize BUY/SELL from this pipeline. `src/service/oandaRun.test.ts` used to carry a test ("proves the safety clamp actually holds something back") that constructed a fully realistic scenario where the underlying pipeline genuinely computed `BUY` and asserted the clamp still forced `WAIT`; that test was rewritten under ADR-018 to assert the opposite.

## ADR-018: Remove the OANDA pipeline's event-risk entry-authorization clamp

Status: Accepted, explicit user decision (2026-07-28).

`safetyConstrainedState` (ADR-016) is removed from `src/service/oandaRun.ts`. `runManualOandaAnalysis` now persists and displays whatever `buildDashboardState` computes directly — including live `BUY`/`SELL` with `isActionable: true`, entry/stop/target prices, and estimated reward-to-risk — for both manual and scheduled OANDA runs (they share the same function; see `src/service/scheduledOandaRun.ts`). This was requested explicitly, with the ADR-016 tradeoff (the event-risk feed, `src/service/forexFactoryEventRisk.ts`, is still an unofficial Forex Factory scrape, not a validated production source per ADR-014) stated and acknowledged before the change. Independently, the same day, the Patience Filter's event-risk and cross-market gates (`src/domain/patienceFilter.ts`) were changed from hard blocking gates to advisory-only — so even before this ADR, those two inputs no longer withheld a signal on their own; ADR-018 removes the second, separate layer that re-blocked OANDA specifically regardless of the Patience Filter's own gates. The trader is now the sole safety check on event-risk exposure for OANDA-sourced signals: the app surfaces the computed decision and the event-risk feed's data (still spike-quality, still unofficial) side by side, and expects the trader to weigh it before entering. `enforceAnalysisSafety` (ADR-015, mock/chart-adapter path only) and the Patience Filter's remaining hard gates (stale data, unavailable provider, open candle, structural invalidation, R:R floor, entry location, confirmation candle, stop/target validity) are unaffected and still apply.
...
- Select a production-grade macro/event-risk data source for the first live release. A validation spike (unofficial Forex Factory feed) is wired for observation only — see ADR-014. Since ADR-018, the OANDA pipeline's entry authorization is no longer hard-blocked pending this — the trader weighs the still-unofficial event-risk data themselves.
...
The implementation remains analysis-only and local-first. OANDA is read-only with server-side credentials; there is no browser-to-OANDA connection or trade execution. H4 and Daily inputs are separate, completed-candle safety is authoritative, saved OANDA reports are immutable, and Chart Preview is on-demand only. The fixture scheduler remains the default; OANDA scheduling is opt-in and now evaluates all six daily H4-close slots (ADR-012). Cross-market confirmation is live (same OANDA account); event-risk is a validation spike only (unofficial feed, not a production commitment); live OANDA observation remains experimental pending broader real-world use. Since ADR-018, the OANDA pipeline persists and displays whatever the deterministic pipeline computes directly, including live BUY/SELL — the former entry-authorization clamp (ADR-016) is removed, and the Patience Filter's event-risk/cross-market gates are advisory-only (see patienceFilter.ts); the trader is the safety check on those two inputs now, not the app. Strategy parameters are configurable and versioned (ADR-017), but no live/scheduled run selects a non-default strategy yet — only the isolated backtest harness does.
