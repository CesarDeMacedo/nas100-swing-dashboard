...
Manual OANDA analysis, immutable saved display snapshots, saved-dashboard review, Chart Preview, PNG export (chart-only), Analysis History search/filter, an OANDA status badge, and optional fixture-default/OANDA-opt-in scheduling are implemented. H4 and Daily inputs remain separate and completed-candle safety is required. Chart navigation preserves the user viewport until explicit Reset view. Both the manual and scheduled OANDA paths fetch retry-bounded H4 candles (C4), live cross-market H4 confirmation for US500/US30/Russell 2000 from the same OANDA account (A1), and a Forex Factory event-risk validation spike (A2); scheduler outcomes trigger a local OS notification via `node-notifier` (A5). Entry authorization from the OANDA pipeline is no longer hard-blocked — ADR-018 removed the former `safetyConstrainedState` clamp in `src/service/oandaRun.ts` (see `docs/DECISIONS.md` ADR-016/ADR-018); the pipeline now persists and displays its real computed decision even though the event-risk provider question remains unresolved for production, not just validated as a spike. Live OANDA observation remains experimental (opt-in, not the default), but shared-subscriber, reconnect/backoff, H4 rollover, saved-candle immutability, and saved-report invariance lifecycle tests are now complete (`src/service/liveStream.test.ts`). The browser never connects directly to OANDA.
...
Manual OANDA analysis is implemented as a separate `POST /runs/manual-oanda` path. It requests 250 midpoint H4 candles and 250 Daily candles, excludes all open candles, and keeps H4 structure/decision inputs separate from Daily Regime inputs. Both source timestamps are persisted with the immutable local report, while H4 remains the run identity. The path is GET-only toward OANDA and manual only; no dashboard or scheduler integration exists. Cross-market confirmation (US500/US30/Russell 2000) is now live, fetched from the same OANDA account and classified against NAS100's own H4 structure (A1, `src/service/oandaRun.ts`'s `fetchCrossMarketH4`); event-risk is a Forex Factory validation spike, not a resolved production input (A2, `src/service/forexFactoryEventRisk.ts`). Since ADR-018 removed the former `safetyConstrainedState` clamp, the resulting report reflects whatever the pipeline actually computes, including live BUY/SELL (see `docs/DECISIONS.md` ADR-016/ADR-018). No trade execution exists.
...
Saved OANDA reports with a non-sensitive display snapshot can now be opened from History in the existing dashboard layout for the current browser session. This is historical review only: neither the scheduler nor browser starts new OANDA loading automatically. On startup, the dashboard shows the most recently saved OANDA report if one exists — a local history lookup only, no new OANDA API call — falling back to mock data when the service is unavailable or nothing has ever been saved.

A manual "Run OANDA analysis now" header control (`OandaManualRunControl`, wired via `runManualOandaNow` in `src/App.tsx`) triggers `POST /runs/manual-oanda` on demand, reusing the existing dedup-by-`runKey` behavior and disabling itself while a request is in flight. On a successful or `already_exists` result it fetches the full saved report via `GET /runs/:runKey` and switches the dashboard straight to it, reusing the same `viewSavedOandaAnalysis` path as the History "View in dashboard" action and the startup auto-load — so the user never has to leave the dashboard to see the result.
