...
export type OandaPreviewData = {
  provider: 'oanda-v20';
  environment: 'practice' | 'live';
  instrument: string;
  timeframe: 'H4';
  candles: OandaPreviewCandle[];
};
export type OandaPreviewResult =
  | { kind: 'succeeded'; data: OandaPreviewData }
  | { kind: 'failed' | 'malformed_response'; message: string };

export type OandaLiveEvent = {
  type: 'connection' | 'snapshot' | 'price' | 'candle' | 'heartbeat' | 'error';
...
export type StrategyParameters = {
  ...
  /** Optional: configs persisted before this parameter existed don't return it; absent means 'deepest'. */
  invalidationAnchor?: 'deepest' | 'traded_zone';
  /** Optional: absent means 'pipeline' (the original gate pipeline). */
  strategyKind?: 'pipeline' | 'rsi2' | 'double7';
  /** Optional: engine parameters for the mean-reversion strategy kinds. */
  meanReversion?: {
    timeframe: 'D' | 'H4';
...
export type StrategyListResult =
  | { kind: 'succeeded'; strategies: StrategyConfig[] }
  | { kind: 'failed'; message: string }
  | { kind: 'malformed_response'; message: string };
export type StrategyDetailResult =
  | { kind: 'succeeded'; strategyId: string; versions: StrategyConfig[] }
  | { kind: 'failed'; message: string }
  | { kind: 'malformed_response'; message: string };
export type StrategyMutationResult =
  | { kind: 'succeeded'; strategy: StrategyConfig }
...
export type BacktestListResult =
  | { kind: 'succeeded'; backtests: BacktestRunSummary[] }
  | { kind: 'failed'; message: string }
  | { kind: 'malformed_response'; message: string };
export type BacktestReportResult =
  | { kind: 'succeeded'; report: BacktestReport }
  | { kind: 'not_found' }
  | { kind: 'failed'; message: string }
  | { kind: 'malformed_response'; message: string };
...
const isPersistedRun = (value: unknown): value is PersistedRun => {
  ...
  return (
    ...
    typeof run.persistedAt === 'string' &&
    (typeof run.reportId === 'string' || run.reportId === null) &&
    (run.strategyName === undefined ||
      typeof run.strategyName === 'string' ||
      run.strategyName === null) &&
    (run.strategyVersion === undefined ||
      typeof run.strategyVersion === 'number' ||
...
const isSavedOandaDisplaySnapshot = (value: unknown): value is SavedOandaDisplaySnapshot => {
  ...
  return (
    snapshot.provider === 'oanda-v20' &&
    (snapshot.environment === 'practice' || snapshot.environment === 'live') &&
    ...
    (typeof snapshot.h4SourceCandleTime === 'string' || snapshot.h4SourceCandleTime === null) &&
    (typeof snapshot.dailySourceCandleTime === 'string' ||
      snapshot.dailySourceCandleTime === null) &&
...
const isImmutableReportDetail = (value: unknown): value is ImmutableReportDetail => {
  ...
  return (
    ...
    typeof report.direction === 'string' &&
    (typeof report.score === 'number' || report.score === null) &&
    (typeof report.grade === 'string' || report.grade === null) &&
    typeof report.primaryReason === 'string' &&
    (typeof report.entryTrigger === 'string' || report.entryTrigger === null) &&
    (typeof report.stopPrice === 'number' || report.stopPrice === null) &&
    Array.isArray(report.targets) &&
    report.targets.every((target) => typeof target === 'number') &&
    (typeof report.estimatedRewardRisk === 'number' || report.estimatedRewardRisk === null) &&
    (typeof report.sourceCandleTime === 'string' || report.sourceCandleTime === null) &&
    typeof report.isActionable === 'boolean' &&
    (report.displaySnapshot === undefined || isSavedOandaDisplaySnapshot(report.displaySnapshot))
  );
};
...
const isBacktestRunSummary = (value: unknown): value is BacktestRunSummary => {
  ...
  return (
    typeof run.id === 'string' &&
    typeof run.strategyConfigId === 'string' &&
    typeof run.instrument === 'string' &&
...
const isBacktestReport = (value: unknown): value is BacktestReport => {
  ...
  const report = value as Record<string, unknown>;
  return (
    isBacktestRunSummary(report.run) &&
    report.summary !== null &&
...
const isOandaPreviewData = (value: unknown): value is OandaPreviewData => {
  ...
  return (
    data.provider === 'oanda-v20' &&
    (data.environment === 'practice' || data.environment === 'live') &&
    typeof data.instrument === 'string' &&
    data.timeframe === 'H4' &&
    Array.isArray(data.candles) &&
    data.candles.every((candle) => {
...
export function createLocalAnalysisServiceClient(
  baseUrl = serviceUrl(),
  ...
  return {
    async checkHealth() {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'unavailable',
            message: serviceErrorMessage(payload, 'Local service is unavailable.'),
          };
        if (!payload || typeof payload !== 'object')
          return invalidResponse('Local service returned an invalid health response.');
        const health = payload as { status?: unknown; persistence?: { available?: unknown } };
        if (health.status !== 'healthy' || health.persistence?.available !== true)
          return invalidResponse('Local service returned an invalid health response.');
        return { kind: 'available' };
      } catch {
        return {
          kind: 'unavailable',
          message: 'Start the local analysis service to enable manual persistence.',
        };
      }
    },
    async checkOandaStatus() {
      try {
        ...
        const status = payload as {
          state?: unknown;
          environment?: unknown;
          configuredInstrument?: unknown;
          message?: unknown;
        };
        if (
          status.state === 'configured' &&
          (status.environment === 'practice' || status.environment === 'live')
        ) {
          return {
            kind: 'configured',
            environment: status.environment,
            configuredInstrument: status.configuredInstrument === true,
          };
        }
        if (
        ...
        if (status.state === 'invalid') {
          return {
            ...
            message:
              typeof status.message === 'string'
                ? status.message
                : 'OANDA configuration is invalid.',
          };
        }
        return { kind: 'unavailable', message: 'OANDA configuration status is unavailable.' };
      } catch {
    ...
    async runManualFixture() {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Manual analysis could not be saved.'),
          };
        if (!isManualRunSummary(payload))
          return invalidResponse('Local service returned an invalid manual-run response.');
        return payload.alreadyExists
          ? { kind: 'already_exists', run: payload }
          : { kind: 'succeeded', run: payload };
      } catch {
        return {
          kind: 'failed',
          message: 'Start the local analysis service to enable manual persistence.',
    ...
    async runManualOanda() {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Manual OANDA analysis could not be saved.'),
          };
        if (!isManualRunSummary(payload))
          return invalidResponse('Local service returned an invalid manual-run response.');
        return payload.alreadyExists
        ...
        return {
          kind: 'failed',
          message: 'Start the local analysis service to enable manual OANDA runs.',
        };
      }
    },
    async listRecentRuns(limit) {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load local analysis history.'),
          };
        if (
          !payload ||
          ...
          return invalidResponse('Local service returned an invalid history response.');
        ...
        if (
          !runs.every(
            (item) =>
              item &&
              typeof item === 'object' &&
              isPersistedRun((item as { run?: unknown }).run) &&
              ((item as { report?: unknown }).report === null ||
                isAnalysisHistorySummary((item as { report?: unknown }).report)),
          )
        )
          return invalidResponse('Local service returned an invalid history response.');
        ...
        return items.length === 0
          ? { kind: 'empty', runs: [] }
          : { kind: 'succeeded', runs: items };
      } catch {
        return {
          kind: 'failed',
          message: 'Start the local analysis service to view analysis history.',
        };
      }
    },
    async getRunByKey(runKey) {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load the stored analysis report.'),
          };
        if (!payload || typeof payload !== 'object')
          return invalidResponse('Local service returned an invalid analysis-detail response.');
        const item = payload as { run?: unknown; report?: unknown };
        if (!isPersistedRun(item.run) || !isImmutableReportDetail(item.report))
          return invalidResponse('Local service returned an invalid analysis-detail response.');
        return {
          kind: 'succeeded',
          item: { run: item.run, report: item.report },
          report: item.report,
        };
      } catch {
        return {
    ...
    async getOandaCandles(count = 250) {
      try {
        const response = await request(
          `/providers/oanda/candles?count=${encodeURIComponent(String(count))}`,
        );
        const payload = await responseJson(response);
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'OANDA chart preview data could not be loaded.'),
          };
        return isOandaPreviewData(payload)
          ? { kind: 'succeeded', data: payload }
          : invalidResponse('Local service returned invalid OANDA candle data.');
      } catch {
        return { kind: 'failed', message: 'Local service is unavailable.' };
      }
    },
    subscribeOandaLiveH4(listener) {
      const source = new EventSource(`${baseUrl}/providers/oanda/live-h4`);
      const events: OandaLiveEvent['type'][] = [
        'connection',
        'snapshot',
        'price',
        'candle',
      ...
      for (const type of events)
        source.addEventListener(type, (event) => {
          try {
            ...
            if (payload && typeof payload === 'object')
              listener({ type, payload: payload as Record<string, unknown> });
            else
              listener({
                type: 'error',
                payload: { state: 'offline', message: 'Malformed live observation event.' },
              });
          } catch {
            listener({
              type: 'error',
      ...
      source.onerror = () =>
        listener({
          ...
          payload: {
            state: 'offline',
            message: 'Local live observation connection is unavailable.',
          },
        });
      return () => source.close();
    },
    async listStrategies(status) {
      try {
        const response = await request(
          status ? `/strategies?status=${encodeURIComponent(status)}` : '/strategies',
        );
        const payload = await responseJson(response);
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load strategies.'),
          };
        if (
          !payload ||
          typeof payload !== 'object' ||
          !('strategies' in payload) ||
          !Array.isArray(payload.strategies) ||
          !payload.strategies.every(isStrategyConfig)
        )
          return invalidResponse('Local service returned an invalid strategies response.');
        return { kind: 'succeeded', strategies: payload.strategies };
      } catch {
        return {
          kind: 'failed',
          message: 'Start the local analysis service to manage strategies.',
        };
      }
    },
    async getStrategy(strategyId) {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load this strategy.'),
          };
        if (
          !payload ||
          typeof payload !== 'object' ||
          typeof (payload as { strategyId?: unknown }).strategyId !== 'string' ||
          !Array.isArray((payload as { versions?: unknown }).versions) ||
          !(payload as { versions: unknown[] }).versions.every(isStrategyConfig)
        )
          return invalidResponse('Local service returned an invalid strategy response.');
        return {
          kind: 'succeeded',
          strategyId: (payload as { strategyId: string }).strategyId,
          versions: (payload as { versions: StrategyConfig[] }).versions,
        };
      } catch {
        return {
          kind: 'failed',
      ...
      try {
        ...
        if (response.status === 422)
          return {
            ...
            message: serviceErrorMessage(payload, 'Strategy parameters are invalid.'),
          };
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not create the strategy.'),
          };
        if (!isStrategyConfig(payload))
          return invalidResponse('Local service returned an invalid strategy response.');
    ...
    async createStrategyVersion(strategyId, name, parameters) {
      try {
        const response = await request(`/strategies/${encodeURIComponent(strategyId)}/versions`, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ name, parameters }),
        });
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not create the new strategy version.'),
          };
        if (!isStrategyConfig(payload))
          return invalidResponse('Local service returned an invalid strategy response.');
        return { kind: 'succeeded', strategy: payload };
    ...
    async activateStrategyVersion(strategyId, version) {
      try {
        const response = await request(
          `/strategies/${encodeURIComponent(strategyId)}/versions/${version}/activate`,
          { method: 'POST' },
        );
        const payload = await responseJson(response);
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not activate this strategy version.'),
          };
        if (!isStrategyConfig(payload))
          return invalidResponse('Local service returned an invalid strategy response.');
        return { kind: 'succeeded', strategy: payload };
    ...
    async listBacktests(strategyConfigId) {
      try {
        const response = await request(
          strategyConfigId
            ? `/backtests?strategyConfigId=${encodeURIComponent(strategyConfigId)}`
            : '/backtests',
        );
        const payload = await responseJson(response);
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load backtest results.'),
          };
        if (
          !payload ||
          typeof payload !== 'object' ||
          !('backtests' in payload) ||
          !Array.isArray(payload.backtests) ||
          !payload.backtests.every(isBacktestRunSummary)
        )
          return invalidResponse('Local service returned an invalid backtests response.');
    ...
    async getBacktest(id) {
      try {
        ...
        if (!response.ok)
          return {
            kind: 'failed',
            message: serviceErrorMessage(payload, 'Could not load this backtest report.'),
          };
        if (!isBacktestReport(payload))
          return invalidResponse('Local service returned an invalid backtest report.');
