...
export function selectOfficialSetupScore(input: { decision: StrategyDecision; longScore: SetupScore; shortScore: SetupScore; longPlan: TradePlan; shortPlan: TradePlan; longPatience: PatienceFilterResult; shortPatience: PatienceFilterResult }, params: ResolvedStrategyParameters = DEFAULT_STRATEGY_PARAMETERS): ScoredDecision {
  ...
  const actionable = (decision.action === 'BUY' || decision.action === 'SELL') && patience.status === 'allowed' && plan.status === 'ready' && (plan.estimatedRewardRisk ?? 0) >= params.minRewardRisk;
  const unavailable = selected.status === 'unavailable';
  const eligible = !unavailable && selected.total >= params.premiumScoreThreshold;
