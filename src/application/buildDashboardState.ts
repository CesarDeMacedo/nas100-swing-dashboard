...
import { DEFAULT_STRATEGY_PARAMETERS, type ResolvedStrategyParameters } from '../domain/strategyParameters';
...
export function buildDashboardState(analysis: AnalysisReport, candleDataset: CandleDataset, technicalContextOverride?: TechnicalContext, params: ResolvedStrategyParameters = DEFAULT_STRATEGY_PARAMETERS): DashboardState {
  ...
  const longPlan = calculateTradePlan({ direction: 'long', technicalContext, latestCandle, preferredEntryZone: analysis.preferredEntryZone, supportZones: analysis.supportZones, resistanceZones: analysis.resistanceZones }, params);
  const shortPlan = calculateTradePlan({ direction: 'short', technicalContext, latestCandle, preferredEntryZone: analysis.preferredEntryZone, supportZones: analysis.supportZones, resistanceZones: analysis.resistanceZones }, params);
  ...
  const longPatience = evaluatePatienceFilter('long', { technicalContext, dataFreshness: analysis.dataFreshness, providerStatus: analysis.dataHealth.providerStatus, eventRisk, crossMarket, ...tradePlanToPatienceInputs(longPlan) }, params);
  const shortPatience = evaluatePatienceFilter('short', { technicalContext, dataFreshness: analysis.dataFreshness, providerStatus: analysis.dataHealth.providerStatus, eventRisk, crossMarket, ...tradePlanToPatienceInputs(shortPlan) }, params);
  const decision = decideStrategy({ technicalContext, longPlan, shortPlan, longPatience, shortPatience }, params);
  const longScore = calculateSetupScore({ direction: 'long', technicalContext, tradePlan: longPlan, patienceFilter: longPatience, crossMarket, eventRisk }, params);
  const shortScore = calculateSetupScore({ direction: 'short', technicalContext, tradePlan: shortPlan, patienceFilter: shortPatience, crossMarket, eventRisk }, params);
  const scored = selectOfficialSetupScore({ decision, longScore, shortScore, longPlan, shortPlan, longPatience, shortPatience }, params);
  return { instrument: analysis.instrument, displayName: analysis.displayName, timeframe: analysis.timeframe, generatedAt: analysis.generatedAt, sourceCandleTime: decision.sourceCandleTime, dataFreshness: analysis.dataFreshness, action: decision.action, actionLabel: actionLabel(decision.action), direction: decision.direction, bias: decision.bias, setupStatus: decision.setupStatus, marketRegime: technicalContext.canonicalDailyRegime, h4Structure: technicalContext.canonicalH4Structure, score: scored.selectedScore, grade: scored.selectedGrade, premiumSetupState: scored.premiumSetupState, isActionable: scored.isActionable, currentPrice: analysis.currentPrice, changePercent: analysis.changePercent, entryTrigger: decision.entryTrigger, entryPrice: decision.entryPrice, invalidationPrice: decision.invalidationPrice, stopPrice: decision.stopPrice, targets: decision.targets, estimatedRewardRisk: decision.estimatedRewardRisk, primaryReason: decision.primaryReason, reasons: decision.reasons, warnings: [...new Set([...decision.warnings, ...scored.warnings])], whyNoEntry: analysis.whyNoEntry, whatToDoNext: analysis.whatToDoNext, marketContext: analysis.marketContext, supportZones: analysis.supportZones, resistanceZones: analysis.resistanceZones, preferredEntryZone: analysis.preferredEntryZone, candles: candleDataset.candles, dataHealth: analysis.dataHealth };
}
