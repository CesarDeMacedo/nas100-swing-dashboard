...
The original pipeline above was dormant in practice (near-zero signal rate over 8 years of backtesting) and its old default-view UI (action banner, setup score, and the sidebar's why-no-entry/next-action/market-context cards) was removed; those older components/tests are still untouched, just unmounted. A newer, simpler live status card (`PipelineStrategyCard`, in `AnalysisSidebar`) was reintroduced after the Patience Filter's event-risk/cross-market gates became advisory-only (raising expected signal rate) and after ADR-018 removed the OANDA pipeline's entry-authorization clamp — it shows the current action/bias/entry/stop/targets/reason from `DashboardState` on the live/mock view, and from the persisted `SafeAnalysis` fields on a saved OANDA snapshot. What the user actually trades day-to-day, alongside this, is Connors-style mean reversion (`strategyKind: 'rsi2' | 'double7'`, pure engine in `src/domain/meanReversionStrategy.ts`), evaluated live on every scheduler slot (`src/service/meanReversionRun.ts`) and shown in a persistent sidebar card + chart overlay + history panel (`MeanReversionStrategyCard`, `mapMeanReversionPriceLines`, `MeanReversionPanel`).
...
2. Cross-market confirmation (A1), scheduler retry/backoff (C4), scheduler notifications (A5), and an event-risk validation spike (A2) are implemented — see `docs/DECISIONS.md` ADR-014/ADR-016/ADR-018. Entry authorization from the OANDA pipeline is no longer hard-blocked (ADR-018 removed the former `safetyConstrainedState` clamp, on explicit user request) — do not reintroduce a clamp on this path, and do not further loosen the Patience Filter's remaining hard gates (stale data, unavailable provider, open candle, structural invalidation, R:R floor, entry location, confirmation candle, stop/target validity — see `patienceFilter.ts`), without explicit, separate approval.
